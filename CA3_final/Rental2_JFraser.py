...
class Dealership(object):
    ...
    def stock_count(self):
        print 'petrol cars in stock ' + str(len(self.petrol_cars))# could print the list using: , self.petrol_cars
        print 'electric cars in stock ' + str(len(self.electric_cars))
        print 'diesel cars in stock ' + str(len(self.diesel_cars))
        print 'hybrid cars in stock ' + str(len(self.hybrid_cars))

    def stock_check(self):
        count = len(self.petrol_cars)+ len(self.electric_cars)+len(self.diesel_cars)+len(self.hybrid_cars)
        return count
...
#        print rent_list		
        ...
        while total < amount:
           ...
           total = total + 1
        print "rented list for this car type: ", rent_list# depending on purpose, this might not be printed, but I include to aid checking/ examiner
        print "customer list for this car type: ", cust_list# as for rent_list, could line be omitted
    ...
    def process_return(self):
        ...
        if answer == 'p':
			...
			# above 3 lines would only be printed if app meant for use by a company operator - would be deleted otherwise; I include as may help for examination/marking purposes!
        self.stock_count()
...
while proceed == 'y':
    ...
    if rent == 'y':
		totalstock = dealership.stock_check()
